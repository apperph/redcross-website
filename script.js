...
// Admin user database (same as in admin-auth.js)
const ADMIN_USERS = [
    {
        id: 1,
...
// Session management for unified login
...
const Login = {
    ...
    isAdminUser: (email, password) => {
        const adminUser = ADMIN_USERS.find(u => 
            u.email === email && 
            u.password === password && 
            u.isActive
        );
    ...
    handleSubmit: async (e) => {
        ...
        try {
            ...
            const adminResult = Login.isAdminUser(formData.email, formData.password);
            
            if (adminResult.success) {
                // Admin login
                SessionManager.createSession(adminResult.user);
                alert(`Welcome back, ${adminResult.user.name}! Redirecting to admin dashboard...`);
                ...
                SessionManager.createSession(regularUser);
                alert('Login successful! Redirecting to your profile...');
                // In a real app, this would redirect to user dashboard
